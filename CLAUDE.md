...
- **新規リポジトリを追加するときは 3 箇所を同時に更新する。** このファイル冒頭の「集約元リポジトリ」リスト（ヘッダ blockquote 内）・末尾の「付録」・`README.md` の「集約元リポジトリ」表のすべてに追記する。1 つでも漏らすと不整合が生じる。**リポジトリ名の載せ忘れ**は `scripts/check_repo_lists.py`（CI で実行。ローカルでは `python3 scripts/check_repo_lists.py`）が 3 箇所を突き合わせて落とす。
- **ただし、この検査は #14 / #15 で起きた種類のずれを防がない。** #14（付録だけ直して README の表が取り残された）と #15 が直したのは、リポジトリ名ではなく**スタック表記とバージョン番号**のずれ（`Next.js 15` と `16`、`EF Core 8` と `9`、実在しない `+ C# .NET 10`）で、検査はそこを一切見ない。書式の約束が箇所ごとに異なるのが正常な状態（README は「静的 HTML/CSS/JS（GitHub Pages）」、付録は「静的 HTML/CSS/JS, GitHub Pages」。語尾も揃っておらず、README の「Docker サンドボックス（bash, Linux）」に対し付録は「Docker サンドボックス, bash, Linux 専用」）で、機械的に揃えることを要求すると正しい記述を落とす方向の指示になるため、あえて対象外にしてある。判定の境界と理由はスクリプト冒頭に明記した。**バージョン表記が各リポジトリの実装とずれていないかは、引き続きレビューで確認する**（本リポジトリは他リポジトリをチェックアウトしないので、そもそも自動では突き合わせられない）。
