...
- **新規リポジトリを追加するときは 3 箇所を同時に更新する。** このファイル冒頭の「集約元リポジトリ」リスト（ヘッダ blockquote 内）・末尾の「付録」・`README.md` の「集約元リポジトリ」表のすべてに追記する。1 つでも漏らすと不整合が生じ、実際 #14（付録だけ直して README の表が取り残された）と #15 で同じ失敗が繰り返された。取りこぼしは `scripts/check_repo_lists.py`（CI で実行。ローカルでは `python3 scripts/check_repo_lists.py`）が 3 箇所を突き合わせて落とす。**この検査が見るのはリポジトリ名の集合と付録の見出し記号の連番だけで、スタック表記やバージョン番号の一致は見ない**（書式や粒度が箇所ごとに異なるのが正常なため。理由と境界はスクリプト冒頭に明記してある）。バージョン表記が実装とずれていないかはレビューで確認する。
