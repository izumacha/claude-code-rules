...
- **スクリーンリーダーに情報を伝える。** 画像に `alt`（装飾画像は `alt=""`）、アイコンだけのボタンに `aria-label`、フォーム入力に対応する `<label>`（または `aria-labelledby`）、動的に変わる領域に `aria-live` を付ける。
...
- **言語属性と外部リンクを正しく設定する。** ルート要素に `lang="ja"` を付け、別タブで開く外部リンクには `rel="noopener noreferrer"` を付ける（§A と整合）。
...
- **フロントの配信を最適化する。** Web ではバンドルを分割（dynamic import / code splitting）し、画像は適切なフォーマット・サイズと `loading="lazy"`（Next.js は `next/image`）で最適化する。重い依存は遅延読み込みする。
