...
- **ただし、この検査は #14 / #15 で起きた種類のずれを防がない。** #14（付録だけ直して README の表が取り残された）と #15 が直したのは、リポジトリ名ではなく**スタック表記とバージョン番号**のずれ（`Next.js 15` と `16`、`EF Core 8` と `9`、実在しない `+ C# .NET 10`）で、検査はそこを一切見ない。書式や粒度が箇所ごとに異なるのが正常な状態（付録は「, GitHub Pages」・README は「（GitHub Pages）」、付録だけが `Auth.js v5` と版を書き README だけが `React 19` を挙げる）で、機械的に揃えることを要求すると正しい記述を落とす方向の指示になるため、あえて対象外にしてある。判定の境界と理由はスクリプト冒頭に明記した。**バージョン表記が各リポジトリの実装とずれていないかは、引き続きレビューで確認する**（本リポジトリは他リポジトリをチェックアウトしないので、そもそも自動では突き合わせられない）。
