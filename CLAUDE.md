...
- **新規リポジトリを追加するときは 3 箇所を同時に更新する。** このファイル冒頭の「集約元リポジトリ」リスト（ヘッダ blockquote 内）・末尾の「付録」・`README.md` の「集約元リポジトリ」表のすべてに追記する。1 つでも漏らすと不整合が生じる。取りこぼしは `scripts/check_repo_lists.py`（CI で実行。ローカルでは `python3 scripts/check_repo_lists.py`）が落とす。**何を検査し、何をあえて検査しないかは同スクリプト冒頭の docstring が唯一の説明**——ここに書き写すと、片方だけ古くなる（このファイルが無くそうとしている形そのもの）。
- **スタック表記が各リポジトリの実装とずれていないかは、引き続きレビューで確認する。** 上の検査は README の表と付録の見出しが互いに食い違っていないかまでしか見ず、両方が揃って古い場合は検出できない（本リポジトリは他リポジトリをチェックアウトしないため、突き合わせる相手が手元に無い）。
